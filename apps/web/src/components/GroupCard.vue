  ...
  <section :class="['group-card','group-'+name,{featured}]">
    ...
    <TransitionGroup name="team-slot" tag="div" class="team-list">
      <div v-for="(team,index) in teams" :key="team.id" class="team-row">
        <span class="num">{{index+1}}</span><span class="team-mark" :class="{'has-logo':team.logoUrl}"><img v-if="team.logoUrl" :src="team.logoUrl" :alt="`โลโก้ ${team.name}`"><template v-else>{{initials(team.name)}}</template></span><span class="team-name">{{team.name}}</span><span v-if="team.seed" class="seed-badge">SEEDED</span>
      </div>
