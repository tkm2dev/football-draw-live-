  ...
  <div class="page draw-control-page">
    ...
    <main class="content admin-layout">
      ...
      <aside class="control-rail">
        <section class="rail-card team-management-card"><div class="rail-icon">✎</div><div class="eyebrow">TEAM MANAGEMENT</div><h3>รายชื่อทีมและกติกา</h3><p>แก้ไขชื่อทีมทั้ง 12 ทีม{{store.divisionKey==='SENIOR40'?' และเลือก 3 ทีมที่ต้องอยู่คนละสาย':''}}</p><button class="btn full" :disabled="busy||!canEditTeams" @click="openTeamEditor">{{drawStarted?'แก้ไขชื่อทีม':'แก้ไขทีมและกติกา'}}</button><small v-if="drawStarted&&!store.locked">เริ่มจับแล้ว: เปลี่ยนชื่อได้ แต่ทีมบังคับจะถูกล็อกไว้</small><small v-if="store.locked">ปลดล็อกผลก่อนจึงจะแก้ไขชื่อทีมได้</small></section>
